      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            <div className="w-9 h-9 rounded-full bg-[#00ff41] p-0.5 shadow-md">
            ...
            <div className="flex flex-col gap-0.8">
              <span className="font-orbitron font-bold text-sm sm:text-lg md:text-xl lg:text-2xl tracking-tighter uppercase italic text-[#00ff41] group-hover:glow-text-green transition-all whitespace-nowrap">
                BADHON'S <span className="text-white">CRACK HUB</span>
              </span>

              <span className="text-xs sm:text-sm md:text-base font-mono tracking-[0.1em] text-[#00ff41]/70 uppercase">
                // OFFICIAL STORE
              </span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`flex items-center space-x-1.5 px-3.5 py-1.5 rounded font-mono text-xs font-bold tracking-wider uppercase transition-all duration-200 ${active
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-3 px-4 py-3 rounded font-mono text-xs uppercase font-bold border transition-all ${active
